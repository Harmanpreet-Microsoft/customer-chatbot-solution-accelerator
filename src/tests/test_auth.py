"""
Comprehensive tests for src/api/app/auth.py
...
def client_principal_token():
    ...
    claims = [
        {"typ": "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress", "val": "test.user@contoso.com"},
...
class TestGetCurrentUserDirectHeaders:
    ...
    async def test_direct_headers_authenticated_user(
        self, mock_get_auth_details, mock_get_user_email, mock_request, client_principal_token
    ):
...
class TestGetCurrentUserErrorHandling:
    ...
    async def test_exception_during_auth_fallback_to_guest(
        ...
        guest_details = {
            "user_principal_id": "guest_fallback",
            "user_name": "guest"
        }
        mock_get_sample_user.return_value = guest_details
    ...
    @pytest.mark.asyncio
    @patch("app.auth.get_user_email")
    async def test_empty_user_name_and_email(
        self, mock_get_user_email, mock_request
    ):
        ...
        headers_dict = {
            "x-ms-client-principal-id": "user_empty"
        }
        mock_request.headers = make_mock_headers(headers_dict)
        mock_get_user_email.return_value = ""
...
class TestGetCurrentUserOptional:
    ...
    async def test_returns_user_when_successful(self, mock_get_current_user, mock_request):
...
class TestRolesAndGuestFlags:
    ...
    async def test_exception_fallback_guest_role(
        ...
        guest_details = {
            "user_principal_id": "exception_guest",
            "user_name": "guest"
        }
        mock_get_sample_user.return_value = guest_details
