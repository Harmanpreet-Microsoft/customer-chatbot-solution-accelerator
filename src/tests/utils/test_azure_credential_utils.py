"""
Tests for azure_credential_utils functions
"""
import os
import sys
...
from app.utils.azure_credential_utils import get_azure_credential, get_azure_credential_async
...
class TestAzureCredentialUtilsAsync:
    ...
    @patch.dict(os.environ, {'APP_ENV': 'dev'})
    async def test_get_azure_credential_async_dev_environment(self, mock_aio_default_cred):
    ...
    async def test_get_azure_credential_async_non_dev_environment(self, mock_aio_managed_cred):
    ...
    async def test_get_azure_credential_async_prod_environment(self, mock_aio_managed_cred):
    ...
    @patch('app.utils.azure_credential_utils.AioManagedIdentityCredential')
    @patch.dict(os.environ, {}, clear=True)
    async def test_get_azure_credential_async_no_env_defaults_to_prod(self, mock_aio_managed_cred):
    ...
    @patch('app.utils.azure_credential_utils.AioManagedIdentityCredential')
    @patch.dict(os.environ, {'APP_ENV': 'prod'})
    async def test_get_azure_credential_async_with_client_id(self, mock_aio_managed_cred):
    ...
    @patch('app.utils.azure_credential_utils.AioDefaultAzureCredential')
    @patch.dict(os.environ, {'APP_ENV': 'dev'})
    async def test_get_azure_credential_async_dev_client_id_ignored(self, mock_aio_default_cred):
...
def test_environment_variable_edge_cases():
    ...
    with patch.dict(os.environ, {'APP_ENV': ''}):
        with patch('app.utils.azure_credential_utils.ManagedIdentityCredential') as mock_managed:
