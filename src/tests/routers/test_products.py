...
class TestProductsEndpoints:
    ...
    @patch('app.routers.products.get_db_service')
    def test_get_products_with_query_parameters(self, mock_get_db, client, sample_product):
    ...
    def test_get_categories_endpoint(self, mock_get_db, client):
        ...
        products = [
            Product(
                id="1", title="P1", price=10, description="D1", category="cat1",
                image="url", rating=4, review_count=1, in_stock=True,
                tags=[], specifications={}, created_at=datetime.utcnow(),
                updated_at=datetime.utcnow()
            ),
            Product(
                id="2", title="P2", price=20, description="D2", category="cat2",
                image="url", rating=4, review_count=1, in_stock=True,
                tags=[], specifications={}, created_at=datetime.utcnow(),
                updated_at=datetime.utcnow()
            ),
            Product(
                id="3", title="P3", price=30, description="D3", category="cat1",
                image="url", rating=4, review_count=1, in_stock=True,
                tags=[], specifications={}, created_at=datetime.utcnow(),
                updated_at=datetime.utcnow()
            )
