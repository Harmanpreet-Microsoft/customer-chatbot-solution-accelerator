...
def test_update_cart_item_success(mock_get_db, mock_get_user):
    ...
    response = client.put("/api/cart/update", params={
        "product_id": "test-product-1",
        "quantity": 5
    })
...
def test_cart_checkout_endpoint(mock_get_db, mock_get_user):
    ...
    cart_with_items = Cart(
        id="cart-123", user_id="test-user-123", items=[
            CartItem(
                product_id="prod-1", product_title="Product 1",
                product_price=10.0, product_image="img.jpg", quantity=2
            )
        ], total_items=2, total_price=20.0
    )
